...
$sql01 = "SELECT * FROM maestro_materia WHERE id_usuario = $idu GROUP BY id_materia";
$result01 = mysql_query($sql01)or die("Error $sql01");
while($field = mysql_fetch_array($result01)){
    $id_materia1 = $field['id_materia'];
    $sql04 = "SELECT * FROM materias WHERE id_materia = $id_materia1";
    $result04 = mysql_query($sql04)or die("No se ejecuta consulta $sql04");
    while($field= mysql_fetch_array($result04)){
    	$nombre=$field['materia'];
